...
func (r *Input) FromMap(values map[string]interface{}) error {
	r.Token = values["token"].(string)
	r.Key = values["key"].(string)
	r.SigningMethod = values["signingMethod"].(string)
	r.Issuer = values["iss"].(string)
	r.Subject = values["sub"].(string)
	r.Audience = values["aud"].(string)
	return nil
}
